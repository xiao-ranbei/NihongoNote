...
### 2026-08-29 晚（续）：成本治理决策（LLM-011/012/013）

用户实测「场景1」（799 字 30 段）后拍板三项成本治理决策：

1. **人工许可才解析（LLM-011，已生效）**：未经用户明确允许，不得发起任何 AI 解析（含回归、批量验证、实测）。需求表与记忆文件均已登记。此前排入的样本回归验证被用户跳过。
2. **内置词典降本（LLM-012，设计已定待实现）**：kuromoji.js 形态素分析 + 解释缓存（固定用法库 + 用户确认回填）+ LLM 兜底三层链路；词典命中 token 瘦身存储。方案见[分析工具化与内置词典降本设计](analysis-tools-design.md)。
3. **分析工具化入口（LLM-013，设计已定待实现）**：分析收敛为设置页式「分析工具」入口，集中管理触发（成本确认弹窗）、词典/AI 混合策略、费用预览、仅词典模式。与后续设置页面一并实现。

**2026-08-29 深夜决策（4 项全部拍板）**：① 形态素分析器用 kuromoji.js；② 解释缓存固定用法库起步 + 用户确认回填（未确认不进缓存）；③ 命中 token 瘦身存储；④ 句段级语义仍走 AI + 词典化兜底，并提供「仅词典分析」零费用模式。问题清单见 [issues.md](issues.md) 第五节。

**消耗量化基准（medium 档，供后续对比）**：两篇文档 35/36 句段，去重后 260,293 tokens（输入 37,329 + 输出 222,964），闲时价约 ¥1.06；342 词条中 particle 102（30%）为固定用法最大浪费点；推理占输出 62%（约 16.2 万 tokens）。
