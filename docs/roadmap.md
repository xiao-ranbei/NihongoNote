...
> 2026-08-29 晚的快照见下方折叠前的历史；本节为 2026-09-02 全量代码审计后的实际状态。
> 审计依据：[项目进展审计](progress-audit-2026-09-02.md)；条目级状态见[问题清单](issues.md#八优先级汇总)。

| 工作流 | 状态 |
| --- | --- |
| P0 代码（骨架 / provider / 校验 / 取消重试） | ✅ 完成。DeepSeek / OpenAI 兼容 / **Ollama 原生** 三套 provider；JSON 解析三层修复链；分析失败三条根因已修（超时阈值、引号退化、服务静默失败） |
| P1 MVP（两层阅读交互 / 手动分析 / 修正 / 学习库） | ✅ 完成。段级修正含人工版本保留（`userRevision`）；学习库搜索 + 分析状态筛选 + 折叠持久化 |
| **P1 后续增强** | ❌ **未动（9 项）**：范围标注 `AnnotationRange`、四层点击循环、等级解释层 `LevelExplanation`、内容类型筛选、移动端抽屉、范围编辑、桌面锚点换位、对话关系箭头。**四项关键词在全仓库检索为 0 命中** |
| 成本治理（超出原计划） | ✅ 三层解析链（词典命中不进 LLM）+ kuromoji 形态素 + 固定用法库（覆盖率 **39.7%**，场景 1 省 363 次 AI 解释调用）；段级档位制（standard 省约 37%）；本地模型零 API 费用 |
| UI / 主题系统 | 🟡 样式 Token 化完成，六主题（`data-theme`）落地；**主题切换 UI 仍未接入**（I-14）。结构层规范已立，见[主题系统](theme-system.md) |
| 自动化测试 / lint | `verify-pipeline.ts` **93 项断言**（分段 / 分词 / 落盘 / 档位 / Ollama / 引号修复 / 设置 profiles），`pnpm --filter @nihongonote/api verify`；vitest 与 ESLint 均未引入 |
| 技术债 | `App.tsx` **1611 行**单一组件（I-13），阻塞全部 P1 后续增强 |

**未实现机能共 28 项**，分组为：P1 后续增强 9 / P2 自用质量 12 / P3 扩展 4 / UI 主题 3。
...
**2026-09-02 复核**：上述 7 条闭环均有代码支撑，MVP 判定成立。此后新增的完成项（本表未覆盖）——
词典三层解析链、kuromoji 形态素、段级字段档位制、Ollama 原生 provider、LLM 设置页与多配置 profiles、
分析工具页（预览 → 成本确认 → 批量）、JSON 解析三层修复链。
完整两篇样本的质量/费用验收仍未跑（需用户批准，遵守 LLM-011）。
...
> **2026-09-02 复核**：以上六项**全部未开始**，代码零实现（范围标注与等级解释层这两个关键词在全仓库检索为 0 命中）。
> 条目化见 `issues.md` 的 I-23 ~ I-26 与 I-29。
> **阻塞关系**：第 1、2、4 项都要求大幅改动阅读器组件，而 `App.tsx` 已膨胀到 1611 行（I-13），
> 建议**先拆组件再动功能**，否则改动不可控。
