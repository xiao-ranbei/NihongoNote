...
用户点击「分析」→ 本地预处理（免费）
  ...
  ├─ ② 解释缓存查表（新增，免费）
  │     命中 → 直接取 explanation/gloss（固定用法库 + 用户确认回填）
  │     未命中 → 进入 AI 候选
...
| --- | --- |
| 来源 | **固定用法库起步 + 用户确认回填** |
| 固定用法库 | 预置助词（particle ~40 条：の/を/が/て/と/に/か/ね/は/まで/で/へ 等，覆盖实测 TOP）+ functional ~30 条 + 常见句末语气模板 |
| 用户确认回填 | 用户点过「这条解释好/准确」的 AI 结果自动沉淀为缓存条目，越用越省；**未确认的解释不进缓存**（防止错误解释固化） |
| 数据格式 | 纯数据 TS/JSON 表；与 `TokenAnalysis` 对齐（surface/category/reading/explanation/gloss 等） |
| 置信度 | 固定用法库命中 `confidence=1.0`、来源标记 `dictionary`；用户回填条目 `confidence=0.9`、来源标记 `dictionary`（可后续修订） |
...
| 决策 2：解释缓存来源 | **固定用法库起步 + 用户确认回填**（未确认解释不进缓存） | I-20 |
...
**4 个优化点**：
1. **usage_json 缺缓存字段**（最该修）：42 条历史数据全部只有 3 字段，缓存字段后加未回填，影响费用估算。修法：一次性迁移脚本，旧数据按未命中处理；
